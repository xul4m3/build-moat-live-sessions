...
async def lifespan(app: FastAPI):
    ...
    if existing is not None:
        ...
        logger.info(
            f"loaded index from {state['config'].kb_index_path}: {len(existing.sections)} sections"
        )
    else:
        logger.info("no existing index; /chat will return 'not indexed' until /index is called")
    yield
    # 沒有特別的 shutdown 工作
...
def build_index() -> IndexResponse:
    ...
    logger.info(f"indexed {files_indexed} files, {len(sections)} sections → {cfg.kb_index_path}")
...
_NOT_INDEXED_MSG = (
    "The knowledge base has not been indexed yet. Please POST /index before asking questions."
)
_CANNOT_CONFIRM_MSG = "I cannot confirm this from the knowledge base."
...
def chat(req: ChatRequest) -> ChatResponse:
    ...
    try:
        ...
        # from exc 保留例外鏈（server log 看得到根因），但 client 只收到乾淨的 503 detail
        raise HTTPException(
            status_code=503,
